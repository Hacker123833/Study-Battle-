...
def generate_pdf_questions(pages, subject, topic, max_questions=100, job_id=None):
    """Generate a larger, cleaner source-grounded MCQ bank without an API key.

    The generator deliberately avoids inventing facts. It builds several question
    families from extracted definitions, terminology, formulas, units, numerical
    statements, relationships, and clean source sentences, then applies strict
    duplicate/distractor checks. This gives much better coverage on long textbooks
    while keeping every answer traceable to the uploaded PDF.
    """
    ...
    # Technical vocabulary is useful for distractors, but keep phrases concise.
    vocab=[]
    for term,definition in facts["definitions"]: vocab += [term, definition]
    for desc,term in facts["terms"]: vocab += [desc, term]
    for quantity,unit in facts["units"]: vocab += [quantity, unit]
    for name,formula in facts["formulas"]: vocab += [name, formula]
    vocab += [w for sent in sentences for w in re.findall(r"\b[A-Za-z][A-Za-z0-9-]{3,30}\b", sent)]
    vocab=list(dict.fromkeys(str(x).strip(" .:;,-") for x in vocab if str(x).strip()))

    def unique_pool(values, correct):
        out=[]; used={normalize_text(correct)}
        for value in values:
            value=str(value).strip(" .:;,-")
            k=normalize_text(value)
            if not k or k in used or len(value)>180: continue
            # Avoid distractors that are essentially the same phrase.
            if any(difflib.SequenceMatcher(None,k,normalize_text(x)).ratio()>=0.94 for x in out):
                continue
            used.add(k); out.append(value)
        return out

    def add(qtext, answer, pool, source_page=0, kind="concept"):
        if len(generated)>=max_questions: return False
        ...
        if len(distractors)<3: return False
        distractors=distractors[:3]
        if not _quality_ok(qtext,answer,distractors): return False
        opts=[answer]+distractors
        random.shuffle(opts)
        generated.append({
            "id":"pdf_"+secrets.token_hex(8),"subject":subject,"topic":topic,
            "q":qtext,"options":opts,"answer":opts.index(answer),
            "source":"uploaded PDF","source_page":source_page,"kind":kind
        })
        return True

    # 1. Definitions: direct and reverse forms.
    defs=facts["definitions"]
    def_pool=[d for _,d in defs]
    term_pool=[t for t,_ in defs]
    for term,definition in defs:
        add(f"According to the uploaded material, what is {term}?", definition, def_pool, kind="definition")
        add(f"Which term is correctly described by this definition: {definition}?", term, term_pool, kind="definition-reverse")

    # 2. Terminology / naming.
    terms=facts["terms"]
    tp=[t for _,t in terms]
    for description,term in terms:
        add(f"What term is used for the concept described as: {description}?", term, tp, kind="terminology")

    # 3. Units and formula recognition.
    units=facts["units"]
    up=[u for _,u in units]
    for quantity,unit in units:
        add(f"What is the SI unit of {quantity}?", unit, up, kind="unit")
    formulas=facts["formulas"]
    fp=[f for _,f in formulas]
    for name,formula in formulas:
        add(f"Which expression represents {name}?", formula, fp, kind="formula")
    ...
    for sent in facts["relations"]:
        ...
        if m:
            ...
            add(f"According to the material, how is {a} related to {b}?",correct,
                ["directly proportional","inversely proportional","independent of","equal to"],kind="relationship")

    # 5. Sentence-completion questions. Prefer meaningful technical phrases.
    technical_terms=[x for pair in defs for x in pair[:1]]+[x for _,x in terms]
    technical_terms=list(dict.fromkeys(x.strip() for x in technical_terms if len(x.strip())>=4))
    for sent in sentences:
        if len(generated)>=max_questions: break
        candidates=[t for t in technical_terms if re.search(r"\b"+re.escape(t)+r"\b",sent,re.I)]
        if not candidates: continue
        answer=max(candidates,key=len)
        qsent=re.sub(r"\b"+re.escape(answer)+r"\b","_____",sent,count=1,flags=re.I)
        pool=technical_terms+[x for x in vocab if len(x)<=70]
        add(f"Complete the statement from the uploaded material: {qsent}",answer,pool,kind="completion")

    # 6. Source-grounded comprehension. Use distinct sentences as answer choices,
    # but only when they are sufficiently different and from the same document.
    clean_statements=[]
    for sent in sentences:
        st=sent.strip().rstrip(".")
        if 55<=len(st)<=260 and normalize_text(st) not in {normalize_text(x) for x in clean_statements}:
            clean_statements.append(st)
    random.shuffle(clean_statements)
    for answer in clean_statements:
        if len(generated)>=max_questions: break
        pool=[x for x in clean_statements if normalize_text(x)!=normalize_text(answer)]
        random.shuffle(pool)
        add("Which statement is directly supported by the uploaded material?",answer,pool,kind="comprehension")

    # 7. High-coverage concept questions from sentences with explicit causal or
    # comparison language. These are more useful than generic trivia.
    trigger=re.compile(r"\b(because|therefore|hence|thus|causes?|results? in|depends? on|increases?|decreases?|greater|smaller|higher|lower|whereas|while|unlike|when)\b",re.I)
    for sent in sentences:
        if len(generated)>=max_questions: break
        if not trigger.search(sent): continue
        words=re.findall(r"\b[A-Za-z][A-Za-z0-9-]{4,}\b",sent)
        if len(words)<2: continue
        # Blank a content word that is not a common connector.
        stop={"which","where","there","these","those","because","therefore","while","whereas","when","result"}
        candidates=[w for w in words if w.lower() not in stop]
        if not candidates: continue
        answer=max(candidates,key=len)
        qsent=re.sub(r"\b"+re.escape(answer)+r"\b","_____",sent,count=1,flags=re.I)
        add(f"Based on the uploaded material, complete the statement: {qsent}",answer,vocab,kind="concept")

    # 8. Numeric/factual recall: if a sentence contains a number, ask which value
    # appears in the source. Distractors are other values from the same document.
    number_values=[]
    for sent in sentences:
        vals=re.findall(r"\b\d+(?:\.\d+)?(?:\s*%|\s*[A-Za-zΩμ]+)?\b",sent)
        for v in vals:
            if len(v)<=35: number_values.append(v.strip())
    number_values=list(dict.fromkeys(number_values))
    for sent in sentences:
        if len(generated)>=max_questions: break
        vals=re.findall(r"\b\d+(?:\.\d+)?(?:\s*%|\s*[A-Za-zΩμ]+)?\b",sent)
        if not vals: continue
        answer=vals[0].strip()
        masked=re.sub(r"\b"+re.escape(answer)+r"\b","_____",sent,count=1)
        add(f"Which value correctly completes the statement from the material? {masked}",answer,number_values,kind="numerical-recall")
...
def _process_pdf_job(job_id, raw, filename, subject, topic, owner):
    try:
        ...
        questions = generate_pdf_questions(pages, subject, topic, max_questions=120, job_id=job_id)
        if not questions:
