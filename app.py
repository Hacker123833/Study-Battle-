...
# Fast/accurate ingestion settings. OCR is the expensive part, so only pages
# that genuinely need it are rendered, then OCR workers run concurrently.
OCR_WORKERS = max(1, min(4, int(os.environ.get("OCR_WORKERS", "3"))))
OCR_DPI_SCALE = float(os.environ.get("OCR_DPI_SCALE", "1.5"))
PDF_CACHE_MAX = 3
PDF_TEXT_CACHE = {}
...
def _ocr_page(page):
    """OCR one page. Kept as a compatibility wrapper for single-page use."""
    if fitz is None:
        return ""
    try:
...
def _ocr_image_bytes(image_bytes):
    """Run Tesseract on rendered bytes. Each worker owns its PIL image."""
    if pytesseract is None or Image is None or not _tesseract_path:
        return ""
    img = None
    try:
        img = Image.open(io.BytesIO(image_bytes))
        # RGB avoids unnecessary alpha processing in Tesseract.
        if img.mode != "RGB":
            img = img.convert("RGB")
        text = pytesseract.image_to_string(img, lang="eng", config="--oem 1 --psm 3") or ""
...
def extract_pdf_text(file_bytes, job_id=None):
    """Fast page-aware extraction: native text first, selective parallel OCR second.

    This intentionally avoids OCR'ing text PDFs. For scanned PDFs, page images are
    rendered once and OCR'd concurrently, while progress is updated as each page
    finishes. Extracted text is cached by SHA-256 so re-uploading the same PDF is
    nearly instant on the same server worker.
    """
    _cleanup_pdf_jobs()
    if fitz is None:
    ...
    with PDF_TEXT_CACHE_LOCK:
        ...
        if cached:
            pages, page_count, ocr_pages = copy.deepcopy(cached)
            if job_id:
                _job_update(job_id, progress=70, stage="cached", current_page=page_count,
                            total_pages=page_count, ocr_pages=ocr_pages,
                            message="✓ PDF already analyzed — reusing extracted text", eta_seconds=0,
                            cached=True)
    ...
    if doc.is_encrypted and not doc.authenticate(""):
        doc.close(); raise ValueError("Password-protected PDFs are not supported unless they open without a password.")
    page_count = len(doc)
    if page_count == 0:
    ...
    started = time.time()
    try:
        # Native extraction is extremely fast and should be the default path.
        for i, page in enumerate(doc):
            native = _page_native_text(page)
            compact = len(re.sub(r"\s+", "", native))
            needs_ocr = compact < 100 and bool(_tesseract_path)
            pages[i] = {"page": i + 1, "text": native, "ocr": False, "chars": len(native)}
            if needs_ocr:
                try:
                    pix = page.get_pixmap(matrix=fitz.Matrix(OCR_DPI_SCALE, OCR_DPI_SCALE), alpha=False)
                    ...
                    pass
            if job_id:
                # Native pages are effectively complete; reserve 60% for OCR/cleanup.
                native_done = i + 1
                base_pct = 8 + int(native_done / page_count * (25 if ocr_tasks else 55))
                _job_update(job_id, progress=min(base_pct, 60), stage="extracting",
                            current_page=native_done, total_pages=page_count, ocr_pages=0,
                            message=f"Reading page {native_done} of {page_count}" + (" • OCR queued" if needs_ocr else ""),
                            eta_seconds=round(max(0, (page_count-native_done) * 0.03)))
    finally:
        doc.close()

    # OCR only the pages that need it, concurrently. Tesseract is an external
    # process, so this gives a large speedup on scanned documents.
    ocr_pages = 0
    if ocr_tasks:
        if not _tesseract_path:
            raise ValueError("This PDF needs OCR, but Tesseract is unavailable on the server.")
        _job_update(job_id, progress=30, stage="ocr", current_page=0, total_pages=page_count,
                    ocr_pages=len(ocr_tasks), message=f"OCR required on {len(ocr_tasks)} page(s) • using {OCR_WORKERS} workers…",
                    eta_seconds=max(1, round(len(ocr_tasks) * 1.0 / OCR_WORKERS))) if job_id else None
        completed = 0
        with ThreadPoolExecutor(max_workers=OCR_WORKERS, thread_name_prefix="pdf-ocr") as pool:
            future_map = {pool.submit(_ocr_image_bytes, image_bytes): idx for idx, image_bytes in ocr_tasks}
            for future in as_completed(future_map):
                idx = future_map[future]
                try: ocr = future.result() or ""
                except Exception: ocr = ""
                native = pages[idx].get("text", "")
                compact = len(re.sub(r"\s+", "", native))
                ocompact = len(re.sub(r"\s+", "", ocr))
                # OCR wins only when it actually adds useful content.
                if ocompact > max(80, compact * 1.20):
                    pages[idx] = {"page": idx + 1, "text": ocr, "ocr": True, "chars": len(ocr)}
                    ocr_pages += 1
                completed += 1
                if job_id:
                    elapsed=max(0.1,time.time()-started)
                    rate=elapsed/max(1,completed)
                    remaining=max(0,len(ocr_tasks)-completed)*rate
                    pct=30+int(completed/max(1,len(ocr_tasks))*35)
                    _job_update(job_id, progress=min(65,pct), stage="ocr", current_page=completed,
                                total_pages=len(ocr_tasks), ocr_pages=ocr_pages,
                                message=f"OCR page {completed} of {len(ocr_tasks)}",
                                eta_seconds=round(remaining))

    pages = _remove_repeated_marginal_text(pages)
    if job_id:
        _job_update(job_id, progress=70, stage="cleaning", message="Cleaning text and removing repeated page furniture…", eta_seconds=1)

    total_chars = sum(int(x.get("chars", 0)) for x in pages)
    ...
    # Small bounded cache: enough to make accidental re-uploads instant without
    # letting a long-running Render worker grow indefinitely.
    with PDF_TEXT_CACHE_LOCK:
        if len(PDF_TEXT_CACHE) >= PDF_CACHE_MAX:
            oldest = next(iter(PDF_TEXT_CACHE))
            PDF_TEXT_CACHE.pop(oldest, None)
        PDF_TEXT_CACHE[digest] = (copy.deepcopy(pages), page_count, ocr_pages)
    return pages, page_count, ocr_pages
...
def _extract_candidate_facts(pages):
    ...
    # Drop obvious OCR noise before question generation. Keeping questionable text
    # out of the candidate pool is more accurate than trying to "fix" it later.
    clean_records=[]
    for r in records:
        txt=r["text"]
        alpha=sum(c.isalpha() for c in txt)
        bad=sum(c in "�¤¦§¨¬®±¶×÷" for c in txt)
        if len(txt) >= 35 and alpha/max(1,len(txt)) >= 0.38 and bad/max(1,len(txt)) < 0.025:
...
def _question_is_duplicate(question, existing):
    """Fast duplicate filter: exact/normalized first, then token overlap, then fuzzy only for close lengths."""
    key=normalize_text(question)
    if not key: return True
    kt=set(key.split())
    if len(key) < 18: return key in {normalize_text(x) for x in existing}
    for old in existing:
        old_key=normalize_text(old)
        if not old_key: continue
        if key == old_key: return True
        ot=set(old_key.split())
        if len(kt) >= 8 and len(ot) >= 8:
            overlap=len(kt & ot)/max(1,min(len(kt),len(ot)))
            if overlap >= 0.92: return True
            # Expensive fuzzy comparison only when lengths and token overlap suggest a near duplicate.
            if overlap >= 0.68 and 0.72 <= len(key)/max(1,len(old_key)) <= 1.38:
                if difflib.SequenceMatcher(None,key,old_key).ratio() >= 0.88: return True
    return False
...
def generate_pdf_questions(pages, subject, topic, max_questions=100, job_id=None):
    """Source-grounded local MCQ engine. Generates diverse candidates, validates them, then selects the best target count."""
    ...
    if job_id:
        elapsed=max(0.1,time.time()-started)
        _job_update(job_id,progress=96,stage="quality_check",message=f"Validated {len(generated)} high-quality questions",eta_seconds=0)
    return generated
...
def _process_pdf_job(job_id, raw, filename, subject, topic, owner, target_questions):
    try:
        ...
        _job_update(job_id,progress=76,stage="generating",message=f"Generating and validating up to {target_questions} source-grounded MCQs…",chunks=len(chunks),eta_seconds=max(1, round(target_questions*0.01)))
...
/* VNext Fast + Premium — final visual layer */
:root{--sb-bg:#050812;--sb-panel:rgba(12,18,31,.88);--sb-panel2:#0c1423;--sb-line:rgba(148,163,184,.14);--sb-text:#f8fbff;--sb-muted:#8798b3;--sb-blue:#6d8cff;--sb-cyan:#36d9e8}
*{box-sizing:border-box}body{background:radial-gradient(800px 420px at 50% -180px,rgba(87,112,255,.24),transparent 70%),radial-gradient(700px 400px at 100% 20%,rgba(54,217,232,.07),transparent 70%),#050812}.container{max-width:1000px;padding:16px 16px 70px}.app-shell{gap:16px}.topbar{padding:11px 8px;border:1px solid rgba(148,163,184,.10);border-radius:18px;background:rgba(6,10,18,.78);box-shadow:0 10px 35px rgba(0,0,0,.22)}.topbar-brand{font-size:19px}.nav-pills{background:#080e19;border-color:rgba(148,163,184,.12);box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}.nav-pill{font-weight:850}.nav-pill.active{background:linear-gradient(135deg,#1c315b,#123142);border-color:#365984;box-shadow:0 5px 18px rgba(48,117,200,.16)}.hero{padding:28px;border-radius:28px;background:linear-gradient(135deg,rgba(25,43,79,.95),rgba(9,16,29,.97) 72%);border-color:rgba(110,140,255,.23);box-shadow:0 22px 65px rgba(0,0,0,.24)}.hero h1{font-size:34px;line-height:1.08}.section-card,.card{background:linear-gradient(180deg,rgba(12,19,32,.96),rgba(7,12,22,.96));border-color:rgba(148,163,184,.13);box-shadow:0 20px 60px rgba(0,0,0,.22)}.section-card{padding:21px}.pdfbox{padding:22px;border-radius:25px;background:linear-gradient(145deg,rgba(17,34,60,.98),rgba(7,13,24,.98));border-color:rgba(100,142,255,.34);box-shadow:0 18px 55px rgba(18,55,110,.18)}.pdf-title{font-size:25px;letter-spacing:-.8px}.quality-pill{background:linear-gradient(135deg,rgba(109,140,255,.12),rgba(54,217,232,.08));border-color:rgba(54,217,232,.28)}.pdf-controls select{font-weight:850}.file-drop{min-height:104px!important;border-color:#4e73d8!important;background:linear-gradient(180deg,rgba(7,15,28,.82),rgba(6,12,22,.62))!important}.file-drop .file-icon{width:42px;height:42px;border-radius:13px;background:linear-gradient(145deg,#173b63,#10253e);display:grid;place-items:center}.pdf-progress-wrap{margin-top:13px;padding:15px;border-radius:18px;border:1px solid rgba(88,132,220,.28);background:rgba(5,12,23,.72);box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}.pdf-progress{height:9px;background:#101d31;border-radius:999px}.pdf-progress #pdf_progress_bar{background:linear-gradient(90deg,#6d8cff,#36d9e8);box-shadow:0 0 18px rgba(54,217,232,.22)}.pdf-progress-meta{margin-top:9px;font-size:11px}.quality-summary{background:linear-gradient(145deg,#0b1728,#07101c);border-color:#263b5e}.topiccatalog,.practice-list{gap:10px}.subjectbox,.practice-set{background:linear-gradient(145deg,#0b1423,#080f1b);border-color:#1e2e47}.modegrid{gap:12px}.mode{min-height:120px;border-color:#213451;background:linear-gradient(145deg,#101a2c,#080f1c);box-shadow:0 8px 24px rgba(0,0,0,.16)}.mode.active{background:linear-gradient(145deg,#172d58,#0b2937);border-color:#4a78bd}.profile-hero{padding:22px;background:linear-gradient(145deg,#152442,#09111e);border-color:#294364}.profile-avatar{box-shadow:0 12px 35px rgba(52,111,210,.2)}.statbox,.profile-stat{background:linear-gradient(145deg,#0c1626,#080f1b);border-color:#1d2d47}.quick-action{background:linear-gradient(145deg,#0d1728,#09111e);border-color:#213451}.quick-action.primary{background:linear-gradient(145deg,#1e3d79,#12374a);border-color:#4169a8}.practice-question{letter-spacing:-.7px}.practice-option,.opt{background:linear-gradient(145deg,#0c1627,#09111e);border-color:#263b5a}.practice-option:hover,.opt:hover{border-color:#4e70aa;background:#0f1c30}.q{letter-spacing:-.7px}.empty-state{background:rgba(7,13,23,.45);border-color:#20314b}.refresh-btn{border-radius:13px}.danger-btn{box-shadow:0 10px 25px rgba(190,18,60,.14)}
@media(max-width:760px){.container{padding:10px 10px 45px}.topbar{border-radius:15px}.hero{padding:22px;border-radius:23px}.hero h1{font-size:29px}.section-card{padding:17px}.pdfbox{padding:17px}.pdf-title{font-size:22px}.modegrid{gap:9px}}
