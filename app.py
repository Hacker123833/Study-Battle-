...
import random, string, threading, time, json, os, hashlib, secrets, re, copy, io, unicodedata, difflib
...
def extract_pdf_text(file_bytes):
    ...
    try:
        for i, page in enumerate(doc):
            ...
            if len(re.sub(r"\s+", "", text)) < 80 and pytesseract is not None and Image is not None and ocr_pages < MAX_PDF_PAGES:
...
def generate_pdf_questions(text, subject, topic, max_questions=100):
    ...
    # Fallback: make a statement-completion MCQ by blanking a meaningful phrase.
    for source in sentences:
        ...
        if len(words) < 7:
            continue
        # Prefer scientific-looking terms and longer content words.
        ...
        if question_sentence == source:
            continue
        # Distractors should be terms, not whole unrelated sentences.
        distractors = [w for w in candidate_answers if len(str(w).split()) <= 8]
        if not _answer_options(anchor, distractors):
            continue
