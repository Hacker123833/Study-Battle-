...
import random, string, threading, time, json, os, hashlib, secrets, re, copy, io, unicodedata, difflib, shutil
...
def split_sentences(text):
    ...
    # Preserve common textbook bullets and break prose into usable facts.
    text = re.sub(r"\n\s*([•●▪◦])\s*", r". ", text)
    ...
    for part in parts:
        ...
        if 30 <= len(part) <= 420 and len(re.findall(r"\b\w+\b", part)) >= 6:
            out.append(part)
    return out
...
def extract_pdf_text(file_bytes):
    ...
    try:
        for i, page in enumerate(doc):
            ...
            # OCR only pages with little/no native text. This handles scanned notes,
            # screenshots and image-only textbook pages without OCRing every page.
            if len(re.sub(r"\s+", "", text)) < 80 and pytesseract is not None and Image is not None and _tesseract_path and ocr_pages < MAX_PDF_PAGES:
                pix = page.get_pixmap(matrix=fitz.Matrix(1.5, 1.5), alpha=False)
                img = Image.open(io.BytesIO(pix.tobytes("png")))
                try:
                    ocr_text = pytesseract.image_to_string(img, lang="eng", config="--psm 6") or ""
            ...
            if extracted_chars >= 700000:
                break
    finally:
        doc.close()

    return clean_pdf_text("".join(chunks)), page_count, ocr_pages
...
def generate_pdf_questions(text, subject, topic, max_questions=100):
    """Create content-grounded MCQs locally, with no API key required.

    The strongest patterns are definitions, named terms, units, formulas and
    explicit cause/effect statements. A sentence-completion fallback is used
    only when the PDF has ordinary prose rather than textbook-style definitions.
    """
    ...
    for sent in sentences:
        ...
        patterns = [
            r"^(?:The\s+)?(.{2,80}?)\s+(?:is|are|means|refers to|denotes|is defined as)\s+(.{10,260})$",
            r"^(?:The\s+)?(.{2,80}?)\s*:\s*(.{10,260})$",
        ]
        for pat in patterns:
            ...
            if m:
                ...
                if 1 <= len(term.split()) <= 10 and len(definition.split()) >= 3:
                    definitions.append((term, definition))
                    candidate_answers.append(definition)
                break
        ...
        if m:
            ...
            candidate_answers.append(unit)

        m = re.search(r"(?:is|are)\s+(?:called|known as)\s+([A-Za-z][A-Za-z0-9()\- ]{1,70})$", clean, re.I)
        if m:
            term = m.group(1).strip(" .")
            description = re.sub(r"\s+(?:is|are)\s+(?:called|known as)\s+.*$", "", clean, flags=re.I).strip(" :,-")
            if term and description:
                called.append((description, term))
                candidate_answers.append(term)

        m = re.search(r"(?:formula|equation|relation)\s+(?:for\s+)?(.{2,80}?)\s*(?:is|:|=)\s*([A-Za-z0-9()\[\]{}+\-*/^√= .]{2,140})$", clean, re.I)
        if m and any(ch in m.group(2) for ch in "=+-*/"):
            name = m.group(1).strip(" :,-")
            formula = m.group(2).strip()
            formulas.append((name, formula))
            candidate_answers.append(formula)

        m = re.search(r"(?:is|are)\s+(?:caused by|due to|because of)\s+(.{3,160})$", clean, re.I)
        if m:
            cause = m.group(1).strip(" .")
            effect = re.sub(r"\s+(?:is|are)\s+(?:caused by|due to|because of)\s+.*$", "", clean, flags=re.I)
            if cause and effect:
                causes.append((effect, cause))
                candidate_answers.append(cause)

    # Use short, distinct answers as distractors; never use the correct answer itself.
    candidate_answers = list(dict.fromkeys(x for x in candidate_answers if 1 < len(str(x)) <= 180))

    def add(qtext, answer, extra=None):
        key = normalize_text(qtext)
        if not key or key in seen or _question_is_duplicate(qtext, existing_stems + [x.get("q", "") for x in generated]):
            return False
        pool = list(extra or []) + candidate_answers
        made = _answer_options(answer, pool)
        if not made:
            return False
        opts, answer_index = made
        generated.append({
            "id": "pdf_" + secrets.token_hex(8),
            "subject": subject,
            "topic": topic,
            "q": qtext,
            "options": opts,
    ...
    for term, definition in definitions:
        add(f"According to the uploaded material, what is {term}?", definition,
            [d for _, d in definitions if d != definition])
        if len(generated) >= max_questions: return generated

    for description, term in called:
        add(f"What term is used for {description}?", term,
            [t for _, t in called if t != term])
        if len(generated) >= max_questions: return generated

    for quantity, unit in units:
        add(f"What is the unit of {quantity}?", unit,
            [u for _, u in units if u != unit])
        if len(generated) >= max_questions: return generated

    for name, formula in formulas:
        add(f"Which expression represents {name}?", formula,
            [f for _, f in formulas if f != formula])
        if len(generated) >= max_questions: return generated

    for effect, cause in causes:
        add(f"According to the uploaded material, what causes {effect}?", cause,
            [c for _, c in causes if c != cause])
        if len(generated) >= max_questions: return generated

    # Fallback A: make a statement-completion MCQ from meaningful terms.
    for source in sentences:
        if len(generated) >= max_questions:
            break
        words = re.findall(r"\b[A-Za-z][A-Za-z0-9()\-]{3,}\b", source)
        if len(words) < 7:
            continue
        stop = {"this","that","these","those","which","where","when","there","their","about","using","from","with","into","also"}
        usable = [w for w in words if w.lower() not in stop]
        if not usable:
            continue
        anchor = max(usable, key=lambda w: (len(w), w[0].isupper()))
        question_sentence = re.sub(r"\b" + re.escape(anchor) + r"\b", "_____", source, count=1, flags=re.I)
        if question_sentence == source:
            continue
        distractors = [w for w in candidate_answers if len(str(w).split()) <= 8]
        add(f"Complete the statement from the uploaded material: {question_sentence}", anchor, distractors)

    # Fallback B: every normal text PDF should still produce grounded MCQs even
    # when it contains prose rather than explicit definitions/formulas. The
    # choices are complete statements taken from the same uploaded material,
    # so the system never invents facts merely to fill an option slot.
    if len(generated) < max_questions and len(sentences) >= 4:
        statement_pool = list(dict.fromkeys(s.strip().rstrip(".") for s in sentences if 35 <= len(s) <= 180))
        random.shuffle(statement_pool)
        for answer in statement_pool:
            if len(generated) >= max_questions:
                break
            others = [x for x in statement_pool if normalize_text(x) != normalize_text(answer)]
            random.shuffle(others)
            if len(others) < 3:
                continue
            n = len(generated) + 1
            add(f"Which statement is supported by the uploaded material? (Question {n})", answer, others[:8])
...
def upload_pdf():
    ...
    subject = str(request.form.get("subject") or "General").strip()[:60]
    topic = str(request.form.get("topic") or os.path.splitext(filename)[0] or "Uploaded PDF").strip()[:100]
    if not subject:
        subject = "General"
    if not topic:
        topic = "Uploaded PDF"
    ...
    try:
        text, pages, ocr_pages = extract_pdf_text(raw)
        if pages > MAX_PDF_PAGES:
            return jsonify(success=False, message=f"This PDF has {pages} pages. The limit is {MAX_PDF_PAGES} pages."), 413
        questions = generate_pdf_questions(text, subject, topic, max_questions=100)
    except ValueError as e:
        return jsonify(success=False, message=str(e)), 400
    except Exception as e:
        return jsonify(success=False, message=f"Could not process this PDF: {e}"), 500

    if not text or len(text.split()) < 20:
        if not _tesseract_path:
            return jsonify(success=False, message="This PDF appears to be scanned/image-only, but the server OCR engine is unavailable. The deployment must include tesseract-ocr; please redeploy the Docker project from the latest ZIP."), 422
        return jsonify(success=False, message="No readable text was found. Try a clearer PDF or a scan with visible text."), 422
    if not questions:
        return jsonify(success=False, message="Text was extracted, but there was not enough structured content to make reliable MCQs."), 422

    owner = clean_name(request.form.get("username"))
    with upload_lock:
        # Final duplicate guard against the existing uploaded question bank.
    ...
    return jsonify(success=True, filename=filename, subject=subject, topic=topic,
                   pages=pages, ocr_pages=ocr_pages, extracted_words=len(text.split()),
                   questions_added=len(new_questions),
                   message=f"Created {len(new_questions)} questions from {filename}.")
...
async function uploadPDF(){
  ...
  const btn=$('pdfUploadBtn'), status=$('pdf_status');
  btn.disabled=true; status.textContent='⏳ Reading PDF and generating questions... This can take a little while for scanned PDFs.';
  try{
    const fd=new FormData();
    fd.append('pdf',file);
    fd.append('subject',$('pdf_subject').value.trim()||'General');
    fd.append('topic',$('pdf_topic').value.trim()||file.name.replace(/\.pdf$/i,''));
    fd.append('username', playerName);
    const r=await fetch('/api/upload_pdf',{method:'POST',body:fd});
    const d=await r.json();
    if(!d.success){status.textContent='❌ '+(d.message||'PDF processing failed.');return;}
    status.textContent=`✅ ${d.message} ${d.ocr_pages?`OCR used on ${d.ocr_pages} page(s).`:''}`;
    $('pdf_file').value='';
    $('pdf_topic').value='';
    await loadCatalog();
    toast(`${d.questions_added} questions added to ${d.subject} → ${d.topic}.`,'success');
  }catch(e){status.textContent='❌ Upload failed. Please try the PDF again.';}
